...
/**
 * Fire-and-forget wrapper — same non-blocking guarantee, no await needed
 * at the call site if the caller prefers not to wait for the INSERT.
 */
export const logActionAsync = (params) => {
  logAction(params).catch((err) => {
    console.error('[audit] async audit log failed:', err?.message ?? err);
  });
...
export default { logAction, logActionAsync, audit };
