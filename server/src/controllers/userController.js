...
const verifyAcademicRefs = async ({ section_id, course_id }) => {
  if (section_id !== null) {
    const { rows } = await query('SELECT 1 AS found FROM sections WHERE id = $1', [section_id]);
    if (rows.length === 0) return `Section with id ${section_id} does not exist.`;
  }
  if (course_id !== null) {
    const { rows } = await query('SELECT 1 AS found FROM courses WHERE id = $1', [course_id]);
    if (rows.length === 0) return `Course with id ${course_id} does not exist.`;
  }
  return null;
};
