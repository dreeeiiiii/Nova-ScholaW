...
describe('announcement endpoints', () => {
  ...
  before(async () => {
    if (!process.env.DATABASE_URL) {
      throw new Error('DATABASE_URL is not set — cannot run announcement tests.');
    }
    ...
    await query('DELETE FROM users WHERE email = ANY($1)', [[ADMIN_EMAIL, TEACHER_EMAIL, TEACHER2_EMAIL, STUDENT_EMAIL, STUDENT2_EMAIL]]).catch(() => {});
    await query('DELETE FROM sections WHERE name IN ($1, $2)', ['Grade 10 - Test', 'Grade 11 - Other']).catch(() => {});
    ...
    const loginRes = await postJson(baseUrl, '/api/auth/login', { email: ADMIN_EMAIL, password: PASSWORD });
    adminToken = (await loginRes.json()).token;

    const loginRes2 = await postJson(baseUrl, '/api/auth/login', { email: TEACHER_EMAIL, password: PASSWORD });
    teacherToken = (await loginRes2.json()).token;

    const loginRes3 = await postJson(baseUrl, '/api/auth/login', { email: TEACHER2_EMAIL, password: PASSWORD });
    teacher2Token = (await loginRes3.json()).token;

    const loginRes4 = await postJson(baseUrl, '/api/auth/login', { email: STUDENT_EMAIL, password: PASSWORD });
    studentToken = (await loginRes4.json()).token;

    const loginRes5 = await postJson(baseUrl, '/api/auth/login', { email: STUDENT2_EMAIL, password: PASSWORD });
    student2Token = (await loginRes5.json()).token;
  });

  after(async () => {
    await query('DELETE FROM announcement_targets WHERE announcement_id IN (SELECT id FROM announcements WHERE author_id = ANY($1))', [[adminId, teacherId, teacher2Id]]).catch(() => {});
    await query('DELETE FROM announcements WHERE author_id = ANY($1)', [[adminId, teacherId, teacher2Id]]).catch(() => {});
    await query('DELETE FROM users WHERE email = ANY($1)', [[ADMIN_EMAIL, TEACHER_EMAIL, TEACHER2_EMAIL, STUDENT_EMAIL, STUDENT2_EMAIL]]).catch(() => {});
    await query('DELETE FROM sections WHERE name IN ($1, $2)', ['Grade 10 - Test', 'Grade 11 - Other']).catch(() => {});
    await query('DELETE FROM courses WHERE code = $1', ['TEST101']).catch(() => {});
  ...
  it('teacher creates class announcement with section_ids + course_ids + student_ids → 201 with correct targets', async () => {
    const res = await postJson(baseUrl, '/api/announcements/class', {
  ...
  it('student sees general + targeted class announcements, but NOT other class announcements', async () => {
    const otherClassRes = await postJson(baseUrl, '/api/announcements/class', {
    ...
    assert.ok(titles.includes('Admin General Announcement'), 'Should see general announcement from admin');
    assert.ok(titles.includes('Teacher General Announcement'), 'Should see general announcement from teacher');
    assert.ok(titles.includes('Class Announcement'), 'Should see targeted class announcement');
    assert.ok(!titles.includes('Other Class Announcement'), 'Should NOT see non-targeted class announcement');
  });
  ...
  it('non-author teacher cannot edit another teacher\'s announcement → 403', async () => {
    const res = await putJson(baseUrl, `/api/announcements/${classAnnouncementId}`, {
      title: 'Hacked Title',
    }, teacher2Token);
  ...
  it('non-author teacher cannot delete another teacher\'s announcement → 403', async () => {
    const res = await deleteJson(baseUrl, `/api/announcements/${classAnnouncementId}`, teacher2Token);
    assert.equal(res.status, 403);
  });

  it('author teacher can delete their own announcement → 200', async () => {
    const res = await deleteJson(baseUrl, `/api/announcements/${classAnnouncementId}`, teacherToken);
    assert.equal(res.status, 200);
    const body = await res.json();
  ...
  it('GET /api/announcements/:id returns 404 for non-existent', async () => {
    const res = await getJson(baseUrl, '/api/announcements/999999', studentToken);
    assert.equal(res.status, 404);
  });

  it('student can view general announcement by id', async () => {
    const res = await getJson(baseUrl, `/api/announcements/${generalAnnouncementId}`, studentToken);
    assert.equal(res.status, 200);
    const body = await res.json();
  ...
  it('student can view targeted class announcement by id', async () => {
    const createRes = await postJson(baseUrl, '/api/announcements/class', {
    ...
    const res = await getJson(baseUrl, `/api/announcements/${announcement.id}`, studentToken);
    assert.equal(res.status, 200);
    const body = await res.json();
  ...
  it('query params: type filter works', async () => {
    const res = await getJson(baseUrl, '/api/announcements?type=general', teacherToken);
    assert.equal(res.status, 200);
    const body = await res.json();
  ...
  it('query params: status filter works', async () => {
    const res = await getJson(baseUrl, '/api/announcements?status=published', teacherToken);
    assert.equal(res.status, 200);
    const body = await res.json();
  ...
  it('query params: limit and offset work', async () => {
    const res = await getJson(baseUrl, '/api/announcements?limit=1&offset=0', teacherToken);
    assert.equal(res.status, 200);
    const body = await res.json();
