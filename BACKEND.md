...
| **gallery** | Media upload, approve/reject, browse (`?featured=true`), search (`?q` + filters), my-uploads, featured toggle (`PATCH /:id/feature`), category reassign on pending (`PATCH /:id/category`) | `gallery_media` (`featured BOOLEAN DEFAULT false`) | `galleryRoutes.js` | `galleryController.js`, `galleryModel.js`, `galleryUpload.js` |
...
| `PATCH /api/gallery/:id/category` | body `{ category_id: int\|null }` | Admin only, **pending-only**. `category_id` must be present as integer `>0` or `null` (missing/string/object → 400). Integer is validated against `categories` via `findCategoryById` → 400 if not exist. `null` clears category (`SET NULL`). Loads media via `findById` (shared with `approve`/`reject` pattern) → 404 if not found, `400 Only pending media can have its category reassigned` if `status !== 'pending'`. Always bumps `updated_at`, calls `galleryModel.updateCategory(id, categoryId)` (`UPDATE gallery_media SET category_id=$1, updated_at=NOW() WHERE id=$2`), audits `gallery.category_update`, returns `{ media }`. Route is `PATCH /:id/category` **before** `GET /:id` to avoid param collision (same pattern as B1.4). |
